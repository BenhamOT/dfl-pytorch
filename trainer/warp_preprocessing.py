...
def warp_by_params(params, img, random_warp, transform, can_flip, border_mode, cv2_inter=cv2.INTER_CUBIC):
    if random_warp:
        img = cv2.remap(img, params['mapx'], params['mapy'], cv2_inter)
    if transform:
        img = cv2.warpAffine(img, params['rmat'], (params['w'], params['w']), borderMode=border_mode, flags=cv2_inter)
